library(dplyr)
library(adegenet) #package for multivariate analysis of genetic markers data.
library(NAM) #nested association mapping
library(dendextend)
library(circlize)
#Parallel computing
library(snow)
...
GD <- read.csv("C:/Users/USER/Downloads/X.txt",header = T)
GD[1:10,1:10]
...
####################################Import Label#########################################################################
data_labels <-read.csv("C:/Users/USER/Downloads/Y.csv")
...
tiff("C:/Users/USER/Downloads/test_dendrogram1.tiff", width = 7, height = 7, units = 'in', res = 300)
par(mar = rep(0,4))
circlize_dendrogram(dend, main = "Colored branches",dend_track_height = 0.85, groupLabels = TRUE)
dev.off()
